...
def wishlist_item_variant_name(item):
    ...
    if item.variant:
        return item.variant.display_product()
...
def wishlist_items_with_availability(items, discounts, local_currency=None):
    ...
    for item in items:
        ...
        if item_info['variant'] is not None:
            availability1 = variant_available(item_info['variant'],
                                                    discounts,
                                                    local_currency)
        else:
            availability1 = availability(item.product,
                                            discounts,
                                            local_currency)
        yield item_info, availability1
